...
class ZTEF663Adapter(ZTEBaseAdapter):
    """
    Dedicated Adapter for ZTE ZXHN F663NV9 / F663NV3A / F663N (GPON/XPON ONT).
    - WAN: GPON WAN (net_gponwan_conf_t.gch) with dynamic IF_INDEX discovery & OMCI unlock
    - Resilient network socket cycling handling
    - Anti-Reset Web Flash lock
    """

    def __init__(self, ip: str, port: int = 80, timeout: int = 3):
        super().__init__(ip, port, timeout)
        self.detected_model = "ZTE ZXHN F663 (XPON/GPON ONT)"
    ...
    def configure_wan(self, wan_config: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        # Primary WAN page for F663 series is net_gponwan_conf_t.gch
                ...
                for prof in sorted_profiles:
                    ...
                    try:
                        ...
                        if is_no_error and not has_fatal_error and r_edit.status_code == 200 and "login_t.gch" not in res_text:
                            is_success = True
                            success_msg = f"WAN F663 updated via Web GUI ({mode} | {target_name or f'Index {target_idx}'} | VLAN {actual_vlan or 'Bawaan'} | User: {user})"
                            break
    ...
    def lock_anti_reset(self, lock_config: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
        """Commit running configuration permanently to Flash on F663."""
        try:
            ...
            return True, "Konfigurasi F663 disimpan permanen ke Flash Storage (Web Commit)"
        except Exception:
            return False, "Gagal mengunci konfigurasi F663"
